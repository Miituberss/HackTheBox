...
- [[#Introduction|Introduction]]
	- [[#Introduction#Enumeration Principles|Enumeration Principles]]
	- [[#Introduction#Enumeration Methodology|Enumeration Methodology]]
- [[#Infrastructure Based Enumeration|Infrastructure Based Enumeration]]
	- [[#Infrastructure Based Enumeration#Domain Information|Domain Information]]
	- [[#Infrastructure Based Enumeration#Cloud Resources|Cloud Resources]]
	- [[#Infrastructure Based Enumeration#Staff|Staff]]
- [[#Host Based Enumeration|Host Based Enumeration]]
	- [[#Host Based Enumeration#FTP|FTP]]
	- [[#Host Based Enumeration#SMB|SMB]]
	- [[#Host Based Enumeration#NFS|NFS]]
	- [[#Host Based Enumeration#DNS|DNS]]
	- [[#Host Based Enumeration#SMTP|SMTP]]
	- [[#Host Based Enumeration#IMAP / POP3|IMAP / POP3]]
	- [[#Host Based Enumeration#SNMP|SNMP]]
	- [[#Host Based Enumeration#MySQL|MySQL]]
	- [[#Host Based Enumeration#MSSQL|MSSQL]]
	- [[#Host Based Enumeration#Oracle TNS|Oracle TNS]]
	- [[#Host Based Enumeration#IPMI|IPMI]]
- [[#Remote Management Protocols|Remote Management Protocols]]
	- [[#Remote Management Protocols#Linux Remote Management Protocols|Linux Remote Management Protocols]]
	- [[#Remote Management Protocols#Windows Remote Management Protocols|Windows Remote Management Protocols]]
...
```shell-session
Miituberss@htb[/htb]$ curl -s https://crt.sh/\?q\=inlanefreight.com\&output\=json | jq .
...
```shell-session
Miituberss@htb[/htb]$ curl -s https://crt.sh/\?q\=inlanefreight.com\&output\=json | jq . | grep name | cut -d":" -f2 | grep -v "CN=" | cut -d'"' -f2 | awk '{gsub(/\\n/,"\n");}1;' | sort -u
...
```shell-session
Miituberss@htb[/htb]$ for i in $(cat subdomainlist);do host $i | grep "has address" | grep inlanefreight.com | cut -d" " -f1,4;done
...
```shell-session
Miituberss@htb[/htb]$ for i in $(cat subdomainlist);do host $i | grep "has address" | grep inlanefreight.com | cut -d" " -f4 >> ip-addresses.txt;done
Miituberss@htb[/htb]$ for i in $(cat ip-addresses.txt);do shodan host $i;done
...
```shell-session
Miituberss@htb[/htb]$ dig any inlanefreight.com
...
```shell-session
Miituberss@htb[/htb]$ sudo apt install vsftpd 
```
...
```shell-session
Miituberss@htb[/htb]$ cat /etc/vsftpd.conf | grep -v "#"
```
...
```shell-session
Miituberss@htb[/htb]$ cat /etc/ftpusers
...
```shell-session
Miituberss@htb[/htb]$ ftp 10.129.14.136
...
```shell-session
Miituberss@htb[/htb]$ ls | grep Notes.txt
...
```shell-session
Miituberss@htb[/htb]$ wget -m --no-passive ftp://anonymous:anonymous@10.129.14.136
...
```shell-session
Miituberss@htb[/htb]$ touch testupload.txt
```
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap --script-updatedb
...
```shell-session
Miituberss@htb[/htb]$ find / -type f -name ftp* 2>/dev/null | grep scripts
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap -sV -p21 -sC -A 10.129.14.136
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap -sV -p21 -sC -A 10.129.14.136 --script-trace
...
```shell-session
Miituberss@htb[/htb]$ nc -nv 10.129.14.136 21
```

  FTP

```shell-session
Miituberss@htb[/htb]$ telnet 10.129.14.136 21
```
...
```shell-session
Miituberss@htb[/htb]$ openssl s_client -connect 10.129.14.136:21 -starttls ftp
...
```shell-session
Miituberss@htb[/htb]$ cat /etc/samba/smb.conf | grep -v "#\|\;" 
...
```shell-session
Miituberss@htb[/htb]$ smbclient -N -L //10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ smbclient //10.129.14.128/notes
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p139,445
...
```shell-session
Miituberss@htb[/htb]$ rpcclient -U "" 10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ for i in $(seq 500 1100);do rpcclient -N -U "" 10.129.14.128 -c "queryuser 0x$(printf '%x\n' $i)" | grep "User Name\|user_rid\|group_rid" && echo "";done
...
```shell-session
Miituberss@htb[/htb]$ samrdump.py 10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ smbmap -H 10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ crackmapexec smb 10.129.14.128 --shares -u '' -p ''
...
```shell-session
Miituberss@htb[/htb]$ git clone https://github.com/cddmp/enum4linux-ng.git
Miituberss@htb[/htb]$ cd enum4linux-ng
Miituberss@htb[/htb]$ pip3 install -r requirements.txt
```
...
```shell-session
Miituberss@htb[/htb]$ ./enum4linux-ng.py 10.129.14.128 -A
...
```shell-session
Miituberss@htb[/htb]$ cat /etc/exports 
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap 10.129.14.128 -p111,2049 -sV -sC
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap --script nfs* 10.129.14.128 -sV -p111,2049
...
```shell-session
Miituberss@htb[/htb]$ showmount -e 10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ mkdir target-NFS
Miituberss@htb[/htb]$ sudo mount -t nfs 10.129.14.128:/ ./target-NFS/ -o nolock
Miituberss@htb[/htb]$ cd target-NFS
Miituberss@htb[/htb]$ tree .
...
```shell-session
Miituberss@htb[/htb]$ ls -l mnt/nfs/
...
```shell-session
Miituberss@htb[/htb]$ ls -n mnt/nfs/
...
```shell-session
Miituberss@htb[/htb]$ cd ..
Miituberss@htb[/htb]$ sudo umount ./target-NFS
```
## DNS
...
```shell-session
Miituberss@htb[/htb]$ dig soa www.inlanefreight.com
...
```shell-session
Miituberss@htb[/htb]$ dig ns inlanefreight.htb @10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ dig CH TXT version.bind 10.129.120.85
...
```shell-session
Miituberss@htb[/htb]$ dig any inlanefreight.htb @10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ dig axfr inlanefreight.htb @10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ dig axfr internal.inlanefreight.htb @10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ for sub in $(cat /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt);do dig $sub.inlanefreight.htb @10.129.14.128 | grep -v ';\|SOA' | sed -r '/^\s*$/d' | grep $sub | tee -a subdomains.txt;done
...
```shell-session
Miituberss@htb[/htb]$ dnsenum --dnsserver 10.129.14.128 --enum -p 0 -s 0 -o subdomains.txt -f /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt inlanefreight.htb
...
```shell-session
Miituberss@htb[/htb]$ cat /etc/postfix/main.cf | grep -v "#" | sed -r "/^\s*$/d"
...
```shell-session
Miituberss@htb[/htb]$ telnet 10.129.14.128 25
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap 10.129.14.128 -sC -sV -p25
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap 10.129.14.128 -p25 --script smtp-open-relay -v
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap 10.129.14.128 -sV -p110,143,993,995 -sC
...
```shell-session
Miituberss@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user user:p4ssw0rd
...
```shell-session
Miituberss@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user cry0l1t3:1234 -v
...
```shell-session
Miituberss@htb[/htb]$ openssl s_client -connect 10.129.14.128:pop3s
...
```shell-session
Miituberss@htb[/htb]$ openssl s_client -connect 10.129.14.128:imaps
...
```shell-session
Miituberss@htb[/htb]$ cat /etc/snmp/snmpd.conf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
Miituberss@htb[/htb]$ snmpwalk -v2c -c public 10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ sudo apt install onesixtyone
Miituberss@htb[/htb]$ onesixtyone -c /opt/useful/seclists/Discovery/SNMP/snmp.txt 10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ sudo apt install braa
#Miituberss@htb[/htb]$ braa <community string>@<IP>:.1.3.6.*   # Syntax
Miituberss@htb[/htb]$ braa public@10.129.14.128:.1.3.6.*
...
```shell-session
Miituberss@htb[/htb]$ sudo apt install mysql-server -y
Miituberss@htb[/htb]$ cat /etc/mysql/mysql.conf.d/mysqld.cnf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p3306 --script mysql*
...
```shell-session
Miituberss@htb[/htb]$ mysql -u root -h 10.129.14.132
...
```shell-session
Miituberss@htb[/htb]$ mysql -u root -pP4SSw0rd -h 10.129.14.128
...
```shell-session
Miituberss@htb[/htb]$ locate mssqlclient
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap --script ms-sql-info,ms-sql-empty-password,ms-sql-xp-cmdshell,ms-sql-config,ms-sql-ntlm-info,ms-sql-tables,ms-sql-hasdbaccess,ms-sql-dac,ms-sql-dump-hashes --script-args mssql.instance-port=1433,mssql.username=sa,mssql.password=,mssql.instance-name=MSSQLSERVER -sV -p 1433 10.129.201.248
...
```shell-session
Miituberss@htb[/htb]$ python3 mssqlclient.py Administrator@10.129.201.248 -windows-auth
...
```shell-session
Miituberss@htb[/htb]$ wget https://download.oracle.com/otn_software/linux/instantclient/214000/instantclient-basic-linux.x64-21.4.0.0.0dbru.zip
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open --script oracle-sid-brute
...
```shell-session
Miituberss@htb[/htb]$ ./odat.py all -s 10.129.204.235
...
```shell-session
Miituberss@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE
...
```shell-session
Miituberss@htb[/htb]$ sudo sh -c "echo /usr/lib/oracle/12.2/client64/lib > /etc/ld.so.conf.d/oracle-instantclient.conf";sudo ldconfig
```
...
```shell-session
Miituberss@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE as sysdba
...
```shell-session
Miituberss@htb[/htb]$ echo "Oracle File Upload Test" > testing.txt
Miituberss@htb[/htb]$ ./odat.py utlfile -s 10.129.204.235 -d XE -U scott -P tiger --sysdba --putFile C:\\inetpub\\wwwroot testing.txt ./testing.txt
...
```shell-session
Miituberss@htb[/htb]$ curl -X GET http://10.129.204.235/testing.txt
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap -sU --script ipmi-version -p 623 ilo.inlanfreight.local
...
```shell-session
Miituberss@htb[/htb]$ cat /etc/ssh/sshd_config  | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
Miituberss@htb[/htb]$ git clone https://github.com/jtesta/ssh-audit.git && cd ssh-audit
Miituberss@htb[/htb]$ ./ssh-audit.py 10.129.14.132
...
```shell-session
Miituberss@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132
...
```shell-session
Miituberss@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132 -o PreferredAuthentications=password
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap -sV -p 873 127.0.0.1
...
```shell-session
Miituberss@htb[/htb]$ nc -nv 127.0.0.1 873
...
```shell-session
Miituberss@htb[/htb]$ rsync -av --list-only rsync://127.0.0.1/dev
...
```shell-session
Miituberss@htb[/htb]$ cat /etc/hosts.equiv
...
```shell-session
Miituberss@htb[/htb]$ sudo nmap -sV -p 512,513,514 10.0.17.2
...
```shell-session
Miituberss@htb[/htb]$ rlogin 10.0.17.2 -l htb-student
...
```shell-session
Miituberss@htb[/htb]$ rusers -al 10.0.17.5
...
```shell-session
Miituberss@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --script rdp*
...
```shell-session
Miituberss@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --packet-trace --disable-arp-ping -n
...
```shell-session
Miituberss@htb[/htb]$ git clone https://github.com/CiscoCXSecurity/rdp-sec-check.git && cd rdp-sec-check
Miituberss@htb[/htb]$ ./rdp-sec-check.pl 10.129.201.248
...
```shell-session
Miituberss@htb[/htb]$ xfreerdp /u:cry0l1t3 /p:"P455w0rd!" /v:10.129.201.248
...
```shell-session
Miituberss@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p5985,5986 --disable-arp-ping -n
...
```shell-session
Miituberss@htb[/htb]$ evil-winrm -i 10.129.201.248 -u Cry0l1t3 -p P455w0rD!
...
```shell-session
Miituberss@htb[/htb]$ /usr/share/doc/python3-impacket/examples/wmiexec.py Cry0l1t3:"P455w0rD!"@10.129.201.248 "hostname"
...
[[Ofensive]]
[[HTB Academy/Resúmenes/footprinting|footprinting]]