- [Windows Event Logs](#windows-event-logs)
  - [Windows Event Logging Basics](#windows-event-logging-basics)
  - [The Anatomy of an Event Log](#the-anatomy-of-an-event-log)
  - [Leveraging Custom XML Queries](#leveraging-custom-xml-queries)
  - [Useful Windows Event Logs](#useful-windows-event-logs)
  - [Practical Exercises](#practical-exercises)
- [Analyzing Evil With Sysmon \& Event Logs](#analyzing-evil-with-sysmon--event-logs)
...
- [Event Tracing for Windows (ETW)](#event-tracing-for-windows-etw)
  ...
  - [ETW Architecture \& Components](#etw-architecture--components)
  - [Interacting With ETW](#interacting-with-etw)
  - [Useful Providers](#useful-providers)
  - [Restricted Providers](#restricted-providers)
  - [References](#references)
- [Tapping Into ETW](#tapping-into-etw)
  - [Detection Example 1: Detecting Strange Parent-Child Relationships](#detection-example-1-detecting-strange-parent-child-relationships)
  - [Detection Example 2: Detecting Malicious .NET Assembly Loading](#detection-example-2-detecting-malicious-net-assembly-loading)
  - [Practical Exercise](#practical-exercise)
- [Get-WinEvent](#get-winevent)
  - [Using Get-WinEvent](#using-get-winevent)
...
![Windows search for 'Event Viewer' showing options: Open, Run as administrator, Open file location, Pin to Start, Pin to taskbar.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image42.png)

![Windows Logs showing Application, Security, Setup, System, and Forwarded Events with event counts and sizes.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image1.png)
...
![Event Viewer displaying DLLHijack logs with details of Sysmon events, including registry value changes and process information.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image89.png)
...
![Event Viewer details for SideBySide, Event ID 35, showing provider, version, level, and creation time.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image4.png)
...
![Event Viewer details showing SystemTime, EventRecordID 1773, ProcessID 636, ThreadID 0, on computer ARASHPARSA2BB9.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image5.png)
...
![Event Viewer log for Event 4624, Microsoft Windows security auditing, showing successful account logon with Security ID SYSTEM, Account Name ARASHPARSA2BB9$, and Logon Type 5.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image6.png)
...
![Event Viewer showing Event 4907, Microsoft Windows security auditing, with audit policy change details for account ARASHPARSA2BB9$, object path, and logon ID 0x3E7.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image9.png)
...
![Event 4907 details: SubjectUserName ARASHPARSA2BB9$, SubjectDomainName WORKGROUP, ObjectType File, ObjectName path, ProcessName SetupHost.exe.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image10.png)
...
![Event IDs 4624 Logon and 4672 Special Logon.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image11.png)

Analyzing the special logon event, we gain insights into token permissions granted to the user upon a successful logon.

![Event 4672, Microsoft Windows security auditing, showing SYSTEM account with special privileges like SeAssignPrimaryTokenPrivilege and SeDebugPrivilege.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image12.png)
...
```bash
Miituberss@htb[/htb]$ xfreerdp /u:Administrator /p:'HTB_@cad3my_lab_W1n10_r00t!@0' /v:[Target IP] /dynamic-resolution
```
...
1. Using the graphical interface of the `Performance Monitor` tool, we can visualize various running trace sessions. A detailed overview of a specific trace can be accessed simply by double-clicking on it. This reveals all pertinent data related to the trace, from the engaged providers and their activated features to the nature of the trace itself. Additionally, these sessions can be modified to suit our needs by incorporating or eliminating providers. Lastly, we can devise new sessions by opting for the "User Defined" category.
    ...
    ![Windows desktop showing Command Prompt with performance tracking context details and Performance Monitor app search result.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image46.png)
    
    ![Performance Monitor window displaying a list of running Event Trace Sessions.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image55.png)
    
2. ETW Provider metadata can also be viewed through the [EtwExplorer project](https://github.com/zodiacon/EtwExplorer).
    
    ![ETW Explorer window showing search results for 'PowerShell' with two providers listed, including GUIDs.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image56.png)
...
![Process Hacker window displaying a list of running processes with details like PID, CPU, and memory usage.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image34.png)
...
![Process list showing spoolsv.exe with PID 2792 and conhost.exe with PID 648.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image35.png)
...
![Desktop showing Process Hacker with running processes, Event Viewer with Sysmon logs, and PowerShell executing a command.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image47.png)
...
![Desktop showing Process Hacker with running processes, PowerShell executing commands, and Command Prompt running SilkETW for event tracing.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image48.png)
...
![Desktop showing Process Hacker with running processes, PowerShell executing commands, and Notepad displaying process details with a search for PID 2508.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image49.png)
...
![Sysmon Event 7 details showing image load information for Seatbelt.exe, including process ID, file path, and signature status.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image77.png) ![Sysmon Event 7 showing image load details for Seatbelt.exe, including process ID, file path, and signature status as valid.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image78.png)
...
![Desktop showing Windows Explorer with Temp folder, Command Prompt running SilkETW, PowerShell executing Seatbelt.exe, and Notepad displaying JSON data.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/216/image79.png)
